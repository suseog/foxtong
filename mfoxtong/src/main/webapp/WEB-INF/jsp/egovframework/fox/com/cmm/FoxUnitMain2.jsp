...
<title>여성들의 아름답고 우아한 생활! 여우통!!</title>

<link rel="stylesheet" href="${pageContext.request.contextPath}/css/egovframework/fox/com/cmm/foxMain.css"/>
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/egovframework/fox/com/cmm/mainStyle.css"/>
...
<script>
function search_local_set(str){
	$('#searchSignguCode').val(str);
	document.foxBsshInfoDefaultVO.action="${pageContext.request.contextPath}/FoxMobileMain.fo";
	document.foxBsshInfoDefaultVO.submit();	
}

function search_keyword(el){
	
/*     if (!($(el).val() == '')) {
        $('.del_ico').css('display', 'block');
    } */
	
	$('#searchMtltyNm').val($('#search').val());
	document.foxBsshInfoDefaultVO.action="${pageContext.request.contextPath}/FoxMobileMain.fo";
	document.foxBsshInfoDefaultVO.submit();	
}
...
<div class="container">
	...
	<div class="main_content">
		...
		<div class="top_docbar">
		      <div class="search">
		       	<c:choose> 
		       	<c:when test="${searchVo.searchCtgryId =='' and  searchVo.searcBrtcCode =='' and searchVo.searchSignguCode =='' and searchVo.searchMtltyNm ==''}">
		        	<input type="text" id="search" name="search" placeholder="상호검색하기" onchange="search_keyword()" value="<c:out value='${searchVo.searchMtltyNm}'/>">
		       	</c:when>
				<c:otherwise>
					<input type="text" id="search" name="search" placeholder="<c:out value='${searchVo.searchCtgryId}'/> <c:out value='${searchVo.searchSignguCode}'/> <c:out value='${searchVo.searchMtltyNm}'/>" onchange="search_keyword()" value="">
		        </c:otherwise>
		        </c:choose>  
		          <em class="logo"></em>
		          <em class="del_ico" onclick="searchClose()"></em>
		          <em class="search_ico" onclick="search_keyword()"></em>
		      </div>
		</div>
		<!-- header 검색창 end -->


	<!-- 카테고리 네비게이션 & 햄버거 바 start  -->
	<div class="slide_wrap">
	</form>
	
	<!-- 카테고리 네비게이션 start  -->
    <div class="left_area swiper mySwiper">
        <ul class="swiper-wrapper slide_ul ">
     	     <c:choose> 
		      	<c:when test="${empty resultCodeList}">
					<li class="com-egovNodata">
		             	<span>가테고리 정보가 없습니다.</span>
	           		</li>			
				</c:when>
				<c:otherwise>
					<c:forEach var="result" items="${resultCodeList}">
					 <li class="slide_item swiper-slide "><span>#<c:out value="${result.codeNm}"/></span><input type="hidden" id="ctgryId" name="ctgryId" value="<c:out value="${result.code}"/>"/></li>
					</c:forEach>
	     	   </c:otherwise>
			</c:choose>
        </ul>
    ...
    <div class="right_area">
        ...
        <div class="side_bar">
            ...
            <div class="slide_btn">
                <div class="left_area swiper mySwiper">
                    <ul class="swiper-wrapper slide_ul">
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                        ...
                        <li class="slide_item swiper-slide"> <span>#필라테스</span> </li>
                            ...
                            <div class="zone_list">
                                <ul>
                                    <li class="zone_item">
                                        <span class="active">대구</span>
                                        <ul class="zone_ul">
                                            <li onclick="search_local_set('27110');">
                                                <span>중구</a></span>
                                            </li>
                                            <li onclick="search_local_set('27140');">
                                                <span>동구</span>
                                            </li>
                                            <li onclick="search_local_set('27170');">
                                                <span>서구</span>
                                            </li>
                                            <li onclick="search_local_set('27200');">
                                                <span>남구</span>
                                            </li>
                                            <li onclick="search_local_set('27230');">
                                                <span>북구</span>
                                            </li>
                                            <li onclick="search_local_set('27260');">
                                                <span>수성구</span>
                                            </li>
                                            <li onclick="search_local_set('27290');">
                                                <span>달서구</span>
                                            </li>
                                            <li onclick="search_local_set('27710');">
                                                <span>달성군</span>
                                            </li>
                                        </ul>
                                    </li>
   ...
   <!-- 카테고리 네비게이션 end  -->
    ...
    <div class="banner">
        <figure>
            <img src="${pageContext.request.contextPath}/images/egovframework/fox/com/cmm/banner.png" alt="">
        </figure>
    </div>
	<!-- 배너 광고 end  -->   
    ...
    <div class="business_list">
        <ul>
            <c:choose> 
	      	<c:when test="${empty resultBsshInfoList}">
				<li class="com-egovNodata">
	             	<span>업체 정보가 없습니다.</span>
          		</li>			
			</c:when>
			<c:otherwise>
				<c:forEach var="bsshInfo" items="${resultBsshInfoList}">
	            <li>
	                <figure>
                    	...
                    	<img src="${pageContext.request.contextPath}/images/egovframework/fox/com/cmm/business_logo.png" alt="">
					  	  ...
					  	  <c:if test="${bsshInfo.bsshPhotoId != ''}">
					  	    <img class="img-round" src="<c:url value='${pageContext.request.contextPath}/cmm/fms/getImage.do'/>?atchFileId=<c:out value='${result.bsshPhotoId}'/>&fileSn=0" class="mcomd-photo"/>
						  </c:if>  
			    		</a>
	                </figure>
	                <div class="text_area">
	                    <div class="left_area">
	                        <div class="title">
	                            <strong><c:out value="${bsshInfo.mtltyNm}"/></strong>
	                            <p><c:out value="${bsshInfo.bsshCmpgnWords}"/></p>
	                        </div>
	                        <div class="comment">
	                            <div class="like">
	                                <span><em></em><c:out value="${bsshInfo.preferCo}"/>개</span>
	                            </div>
	                            <div class="reply">
	                                <span><em></em><c:out value="${bsshInfo.postscriptCo}"/>개</span>
	                            </div>
	                        </div>
	                    </div>
	                    <div class="right_area">
	                       <!--  <span>4.2km</span> -->
	                        <span><c:out value="${bsshInfo.dongCodeNm}"/></span>
	                    </div>
	                </div>
	            </li>
	            </c:forEach>
    ...
    <div class="power_link">
        <div class="left_area">
            <div class="title">
                <strong>효과적인 다이어트 필라테스 오늘 가입하면 10% 할인이벤트</strong>
                <p>헤라필라테스</p>
            </div>
            <div class="btn">
        ...
        <div class="right_area">
            <figure>
                <img src="${pageContext.request.contextPath}/images/egovframework/fox/com/cmm/business_logo_02.png" alt="">
            </figure>
        </div>
    </div>
	<!-- 상품광고 start  --> 	
            ...
            <div class="inner">
                <ul>
                    ...
                    <li class="community bottom_link">
                        <a href="${pageContext.request.contextPath}/cop/bbs/anonymous/selectBoardList.fo?bbsId=BBSMSTR_000000000001" rel="external">
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/egovframework/fox/com/cmm/foxMain.js"></script>   
<script>
$(document).ready(function(){
	
	var ctry = "<c:out value='${searchVo.searchCtgryId}'/>";
	
	if(ctry != ''){
	  $('.slide_item').each(function(){ 
	    	if($(this).find('input').val() == ctry){
