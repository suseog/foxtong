    ...
    <title><c:out value='${result.bbsNm}'/> - 글조회</title> 
		...
		<script type="text/javascript">
			...
			function fn_egov_delete_notice() {
				...
				jConfirm('삭제하시겠습니까?', '알림', 'a', function(r) {
					if(r){
					document.frm.action = "${pageContext.request.contextPath}/cop/bbs${prefix}/deleteBoardArticle.fo";
					document.frm.submit();
					 }
				});		
			}
			...
			function fn_egov_moveUpdt_notice() {
				...
				if ("<c:out value='${anonymous}'/>" == "true"){
					...
					document.frm.action = "${pageContext.request.contextPath}/cop/bbs${prefix}/forUpdateBoardArticle.fo";
					document.frm.submit();	
			}
			
			function fn_egov_addReply() {
				document.frm.action = "${pageContext.request.contextPath}/cop/bbs${prefix}/addReplyBoardArticle.fo";
				document.frm.submit();
			...
			function fn_egov_regist_comment() {
				...
				if(document.comment.commentNo.value != ""){
					document.comment.action = "${pageContext.request.contextPath}/cop/cmt${prefix}/updateComment.fo";
				}else{
					document.comment.action = "${pageContext.request.contextPath}/cop/cmt${prefix}/insertComment.fo";
				}
				  ...
				  <c:if test="${anonymous == 'true'}">
					...
					jConfirm('삭제하시겠습니까?', '알림', 'a', function(r) {
						if(r){
							...
							document.commentList.action = "${pageContext.request.contextPath}/cop/cmt${prefix}/deleteComment.fo";
		...
		<script type="text/javascript">
			function fn_egov_addScrap() {
				document.frm.action = "${pageContext.request.contextPath}/cop/scp/addScrap.fo";
				document.frm.submit();			
			}
		</script>
		</c:if>
...
<div data-role="page" id="NoticeInire" data-url="page_main" >
	...
	<div data-role="header">
		<a href="javascript:fn_egov_select_noticeList()" class="ui-btn ui-corner-all ui-icon-arrow-l ui-btn-icon-notext">뒤로</a>
	...
	<div data-role="content" class="com-copContent">
		<div class="com-notSubject">
			<span><label for="name-d"><strong><c:out value="${result.nttSj}"/> (주제변경 )</strong></label></span>
		</div>
		...
		<c:if test="${not empty result.atchFileId}">
		  <c:if test="${result.bbsAttrbCode == 'BBSA02'}">
				<c:import url="/cmm/fms/selectImageFileInfs.fo" charEncoding="utf-8">
	        ...
	        <table width="100%">
		        ...
		        <c:if test="${anonymous == 'true'}">
					<td width="40%" align="right">
						<span style="font-size:11pt">비밀번호:</span>
					</td>
					<td width="20%" align="right">
						<input type="password" name="password" id="password" size="10" maxlength="5"/>
					<td>
				</c:if>
					<td width="40%" align="right">
						<div data-role="controlgroup" data-type="horizontal">
						<a href="javascript:fn_egov_moveUpdt_notice()" class="ui-btn ui-corner-all ui-icon-edit ui-btn-icon-notext"  data-ajax="false">수정</a>
						<a href="javascript:fn_egov_delete_notice()" class="ui-btn ui-corner-all ui-icon-delete ui-btn-icon-notext">삭제</a>
						</div>
					<td>
				</tr>
			</table>
			...
			<table width="100%">
				<tr>
					<td width="50%" align="left">
						<a href="javascript:fn_egov_insertComment()" data-role="none" class="com-egovBtn1">공감하기(팝업패널작성 필요 )</a>
					</td>
					<td>
		...
		<c:if test="${useComment == 'true'}">
			<c:import url="/cop/cmt${prefix}/selectCommentList.fo" charEncoding="utf-8">
