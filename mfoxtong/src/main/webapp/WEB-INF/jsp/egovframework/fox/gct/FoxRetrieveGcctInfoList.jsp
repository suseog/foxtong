		...
		geocoder.addressSearch(adres, function(result, status) {
		    ...
		    // 정상적으로 검색이 완료됐으면 
		     if (status === kakao.maps.services.Status.OK) {	
		        ...
		        // 결과값 위경도 좌표 표				       
		       document.getElementById(elementid).value = coords;				       
		     }
		});
	}
</script>  
  ...
  $(document).ready(function() {
      $('.img-round').roundImage();  
		...
		if (navigator.geolocation) {
			...
			document.foxGcctInfoVO.nowlatitude.value = lat ;
			document.foxGcctInfoVO.nowlongitude.value = lon ; 
			});
		}
	...
	function fn_CupnDetail(stresntlId, strgcctId, strmtltyNm, strbsshEsntlId) {
		var retVal = confirm(strmtltyNm +"업체상세화면으로 가시겠습니까?");
		   if( retVal == true ){				
				document.foxGcctInfoVO.esntlId.value=stresntlId;	
				document.foxGcctInfoVO.gcctId.value=strgcctId;
				document.foxGcctInfoVO.mtltyNm.value=strmtltyNm;				
				document.foxGcctInfoVO.bsshEsntlId.value=strbsshEsntlId;		
				document.foxGcctInfoVO.action = "${pageContext.request.contextPath}/fox/gct/retrieveGcctInfo.fo";
				document.foxGcctInfoVO.submit();
			...
			<table border="1" width = "100%">
				<tr>
					<td width = "20%" align="right">
						<input type="button" onClick="javascript:fn_addCupn();" name="addCupn" id="addCupn" value="업체 쿠폰 등록">					
					</td>
					<td width = "70%">
					...
					<td width = "10%">
						<button type ="submit" onclick="javascript:fn_search()">내주위</button>
					</td>			
				</tr>
			</table>
		</div>
    ...
    <div class="container">
        ...
        <div class="main_content">
            ...
            <form name = "frmCupn" id ="frmCupn" method="post" >
                ...
                <ul>                       
                    <li>
                        <div class="li_inner" id="li_inner" name="li_inner" onclick="javascript:fn_CupnDetail('<c:out value='${logvo.esntlId}'/>','<c:out value='${result.gcctId}'/>','<c:out value='${result.mtltyNm}'/>', '<c:out value='${result.bsshEsntlId}'/>');">
                            <div class="left_area">
                                <div class="title">
                                    <strong>${result.mtltyNm}</strong>
                                    <span>${result.dongCode}</span>
                                    <strong></strong>
                                    <span><em>거리:${result.distance}</em></span>                                    
                                </div>
                                <div class="content">
                                		 <p>12개월 회원권 등록 시 1개월 무료</p>
                                    <p><em>할인율 : </em>${result.gcctAmount} %</p>
                                    <span>${result.distbEndDe}까지</span><br>
                                    <span>${result.adRes}</span><br>
                                    <span><em>지번 : </em>${result.laadRes}</span><br>
                                    
                                    	<input type = "hidden" id="${result.bsshEsntlId}" name = "${result.bsshEsntlId}" value =""> <br>
                                    	<script>
                                    			kkkk('<c:out value= "${result.laadRes}"/>', '<c:out value= "${result.bsshEsntlId}"/>');
                                    	</script> 
                                    <c:if test="${result.tlphonNo != ''}">
	                                    <span>전화번호 : ${result.tlphonNo}</span>
	                                     <a href='tel:${result.tlphonNo}'>Call</a>
                                    </c:if> 
                        ...
                        <div class="btn_wrap">
	                        <c:if test="${logvo.esntlId != ''} && ${logvo.esntlId ne null} && ${result.emplyrgcctflag != ''} && ${result.emplyrgcctflag ne null}">
	                            <button type="button" onclick="javascript:fn_deleteMyCupn('<c:out value='${logvo.esntlId}'/>','<c:out value='${result.gcctId}'/>','<c:out value='${result.mtltyNm}'/>', '<c:out value='${result.bsshEsntlId}'/>');">쿠폰삭제</button>
	                        </c:if>
	                         <c:if test="${result.emplyrgcctflag == '' || result.emplyrgcctflag eq null}">
	                            <button type="button" onclick="javascript:fn_createMyCupn('<c:out value='${logvo.esntlId}'/>','<c:out value='${result.gcctId}'/>','<c:out value='${result.mtltyNm}'/>', '<c:out value='${result.bsshEsntlId}'/>');">쿠폰받기</button>
	                        </c:if>
	                        <%-- 업체id 받을경우 사용예정
	                        <c:if test="${logvo.bsshEsntlId ne null} && ${logvo.userSe =='01'}"> --%>
	                        <c:if test="${logvo.userSe =='01'}">
		                        <button type="button" onclick="javascript:fn_updateCupn('<c:out value='${logvo.esntlId}'/>','<c:out value='${result.gcctId}'/>','<c:out value='${result.mtltyNm}'/>', '<c:out value='${result.bsshEsntlId}'/>');">업체쿠폰수정</button> 
								<button type="button" onclick="javascript:fn_deleteCupn('<c:out value='${logvo.esntlId}'/>','<c:out value='${result.gcctId}'/>','<c:out value='${result.mtltyNm}'/>', '<c:out value='${result.bsshEsntlId}'/>');">업체쿠폰삭제</button>
                        	</c:if>
                        </div>
                    </li>                               
                </ul>
