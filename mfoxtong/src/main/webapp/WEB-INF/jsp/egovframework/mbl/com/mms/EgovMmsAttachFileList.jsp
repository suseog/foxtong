<% 
/**
 * @Class Name : EgovMmsAttachFileList.jsp
 * @Description : 첨부파일 목록조회
...
<script type="text/javascript">
	...
	function press(event) {
		if (event.keyCode==13) {
			fn_egov_select_mmsAttachFile('1');
		}
	}
	
	function fn_egov_select_mmsAttachFile(pageNo) {
		document.frm.pageIndex.value = pageNo;
		document.frm.action = "<c:url value='/mbl/com/mms/selectMmsAttachFileList.mdo'/>";
		document.frm.submit();
	}
	
	function fn_egov_inqire_mmsAttachFile(sn) {
		document.frm.sn.value = sn;
		document.frm.action = "<c:url value='/mbl/com/mms/selectMmsAttachFile.mdo'/>";
		document.frm.submit();
	}

	function fn_egov_regist_mmsAttachFile(){
	    document.frm.action = "<c:url value='/mbl/com/mms/goMmsAttachFileRegist.mdo'/>";
	    document.frm.submit(); 
	}
</script>
...
<noscript class="noScriptTitle">자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다.</noscript>
	...
	<div id="attachFile" class="contents2">
		...
		<tbody>
		 <tr>
		  <td width="40%" class="title_left">
		   <img src="<c:url value='/images/egovframework/mbl/mcomd/icon/tit_icon.gif'/>" width="16" height="16" hspace="3" style="vertical-align: middle" alt="">&nbsp;MMS 첨부파일 목록</td>
			<td width="10%" >
		  ...
		  <th width="10%">
		   <table border="0" cellspacing="0" cellpadding="0">
		    <tr>
		      <td><span class="button"><input type="submit" value="조회" onclick="javascript:fn_egov_select_mmsAttachFile('1'); return false;"></span></td>
		      <td>&nbsp;&nbsp;</td>
		      <td>
		           <span class="button">
		           <a href="<c:url value='/mbl/com/mms/goMmsAttachFileRegist.mdo'/>"
		              onclick="javascript:fn_egov_regist_mmsAttachFile(); return false;"><spring:message code="button.create"/></a>
		           </span>
		      </td>     
		      <td>&nbsp;&nbsp;</td>
		    </tr>
		...
		<table width="100%" cellpadding="8" class="table-list" summary="MMS 첨부파일에 대한 목록을 제공합니다.">
		 <thead>
		  <tr>
		    <th class="title" width="20%" nowrap>번호</th>
		    <th class="title" width="30%" nowrap>제목</th>
		    <th class="title" width="30%" nowrap>파일명</th>
		    <th class="title" width="20%" nowrap>수정일시</th>  
		  </tr>
		 </thead>    
		 <tbody>
			 <c:forEach var="result" items="${resultList}" varStatus="status">
			  <tr>
			    ...
			    <td class="lt_text3" nowrap>
			    	<a href="javascript:fn_egov_inqire_mmsAttachFile('<c:out value="${result.sn}"/>')">
			    	<c:out value="${result.mmsAtchFileSj}"/></a>
			    </td>
			    <td class="lt_text3" nowrap><c:out value="${result.atchFileNm}"/></td>
			    <td class="lt_text3" nowrap><c:out value="${result.updtDt}"/></td>
			  </tr>
			 </c:forEach>	  
		...
		<div align="center">
			<ui:pagination paginationInfo="${paginationInfo}" type="image" jsFunction="fn_egov_select_mmsAttachFile"/>
		</div>
		</form>
	</div>
