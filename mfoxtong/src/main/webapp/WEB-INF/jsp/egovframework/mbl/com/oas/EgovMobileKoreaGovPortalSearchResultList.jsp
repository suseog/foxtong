<% 
/**
 * @Class Name : EgovMobileKoreaGovPortalSearchResultList.jsp
	...
	<head>
		...
		<script type="text/javaScript" language="javascript" defer="defer">
			...
			function initList() {
				...
				$('#searchConditionText').text(fn_egov_get_conditionNm($('#searchCondition').val()));

                $.getJSON("<c:url value='/mbl/com/oas/selectKoreaGovPortalSearchResultList.mdo'/>", $('#searchVO').serialize().replace(/%/g,'%25'), function(json){
					...
					for (var i = 0; i < collectionSet.length; i++) {
						html += '<li><span>';
						html += '	<a href="javascript:fn_egov_search_choice(\'' + collectionSet[i] +'\', \'' + collectionName[i] + '\');" data-role="none">' + collectionName[i] + '</a>';
						html += '</span></li>';
				    }
		            ...
		            // 검색 결과가 존재할 시
						...
						html += '<p class="inquiry">' + fn_egov_get_conditionNm(json.searchVO.searchCondition) + ' <span>['  + + json.resultCnt + '건]</span></p>';
						...
						for ( var i = 0; i < json.titleList.length; i++) {
                            ...
                            if (json.searchVO.searchCondition == 'pubword') {
                            	html += '<li>';
                            	html += '	<a href="javascript:fn_egov_inqire_administWordDetail(\'' + json.titleList[i] + '\', \'' + json.contentsList[i] + '\');">';
					...
					$("#pageUnit option[value='" + json.searchVO.pageUnit + "']").attr('selected', 'selected');
			 ...
			 * 검색조건 toggle
			 ******************************************************** */
			function fn_egov_toggle_searchSelect(){
                $('#searchSelect').toggle('selectDisplay');
			}
			 ...
			 * 행정용어 상세조회
			 ******************************************************** */
			function fn_egov_inqire_administWordDetail(title, contents) {
				$.mobile.changePage( "<c:url value='/mbl/com/oas/goAdministrationWordDetail.mdo'/>", {
					type: "post",
					...
					<div id="searchView">
		            	<div id="search">
							<fieldset>
								...
								<button type="button" id="searchSubmit" data-role="none" onclick="javascript:fn_show_list(1);"><span><strong>검색</strong></span></button>							
							</fieldset>
						</div>
						<div class="searchTerms">
			            	...
			            	<div class="termsRight">
			            		<select id="pageUnit" name="pageUnit" data-role="none">
